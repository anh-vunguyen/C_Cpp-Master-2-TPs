...
Image2D::Image2D(char* path,double milx, double mily){
    ...
    for (int i=0; i<(*this).nby; i++) {
        for (int j=0; j<(*this).nbx; j++) {
            double mean=0;
            mean=((double)imageRED[i*((*this).nbx)+j]+(double)imageBLU[i*((*this).nbx)+j]+(double)imageGRE[i*((*this).nbx)+j])/3.0;
...
void Image2D::alloue_partie_dynamique(int dimx, int dimy){
    ...
    ptr = new int* [dimy]; //!< ptr[i] pointe sur la ligne i.
    for (int i=0; i<dimy ; i++){
        ptr[i] = new int[dimx]; //!< ptr[i][j] sur l'element i,j
...
// TP4 C++
// Chercher les pics dans l'histogramme
vector<int> Image2D::find_peaks(int* h){
    vector<int> v;
    ...
    for(int i=0; i<length; i++){
        bool ispeak = true;
        if(i-1 > 0){
            ispeak = ispeak && (h[i] > 1.8 * h[i-1]);
        }
        if(i+1 < length){
            ispeak = ispeak && (h[i] > 1.8 * h[i+1]);
        }
        ispeak = ispeak && (h[i] > 0.05 * max);
        if(ispeak){
            v.push_back(i);
        }
    }
...
// Segmentation automatique avec l'histogramme
// En niveau gris
void Image2D::segmentation_avec_histogramme(){
    int* h;
    h = (*this).hist();
    vector<int> peaks = (*this).find_peaks(h);
    for(int a=0; a<peaks.size(); a++){

        // Initialisation du pointeur temporel
        int** tmp_ptr;
        tmp_ptr = new int* [(*this).nby];
        for (int i=0; i<(*this).nby ; i++){
            tmp_ptr[i] = new int[(*this).nbx];
        ...
        for (int i=0; i< (*this).nby; i++) {
            for (int j=0; j < (*this).nbx; j++) {
                if(((*this).ptr[i][j] > (val-5)) && ((*this).ptr[i][j] < (val+5))){
                    tmp_ptr[i][j] = (*this).ptr[i][j];
                }else{
                    tmp_ptr[i][j] = 0;
                }
            }
        ...
        int** true_ptr = (*this).ptr;
        (*this).ptr = tmp_ptr;

        // Enregistrement l'image segmentee
        string name =  to_string(val) + ".bmp";
        char *name_ch = &name[0];
        (*this).EnregistrerImage(name_ch);
        (*this).ptr = true_ptr;
    }
}

// En RGB
void Image2D::segmentation_avec_histogrammeRGB(){
    int* h;
    h = (*this).hist();
    vector<int> peaks = (*this).find_peaks(h);
    for(int a=0; a<peaks.size(); a++){
        ...
        for (int i=0; i< (*this).nby; i++) {
            for (int j=0; j < (*this).nbx; j++) {
                if(((*this).ptr[i][j] > (val-5)) && ((*this).ptr[i][j] < (val+5))){
                    tmp_ptr[i][j] = (*this).ptr[i][j];
                    tmp_ptrR[i][j] = (*this).R[i][j];
                    tmp_ptrG[i][j] = (*this).G[i][j];
                    tmp_ptrB[i][j] = (*this).B[i][j];
                }else{
                    tmp_ptr[i][j] = 0;
                    tmp_ptrR[i][j] = 0;
                    tmp_ptrG[i][j] = 0;
        ...
        int** true_ptr = (*this).ptr;
        int** true_ptrR = (*this).R;
        int** true_ptrG = (*this).G;
        int** true_ptrB = (*this).B;
        ...
        // Enregistrement l'image segmentee
        string name =  to_string(val) + "RGB.bmp";
        char *name_ch = &name[0];
        (*this).EnregistrerImageRGB(name_ch);
        (*this).ptr = true_ptr;
        (*this).R = true_ptrR;
        (*this).G = true_ptrG;
        (*this).B = true_ptrB;
...
// Segmentation lignes
void Image2D::segmentation_ligne(){
    ...
    float** kernel;
    kernel = new float*[3];
    kernel[0] = {new float[3]{-1.0, -2.0, -1.0}};
    kernel[1] = {new float[3]{0.0, 0.0, 0.0}};
    kernel[2] = {new float[3]{1.0, 2.0, 1.0}};
    (*this).appliqueFiltre(kernel);
    (*this).EnregistrerImage("horizontale.bmp");

    (*this).ptr = true_ptr;

    // Segmentation verticale
    kernel[0] = {new float[3]{-1.0, 0.0, 1.0}};
    kernel[1] = {new float[3]{-2.0, 0.0, 2.0}};
    kernel[2] = {new float[3]{-1.0, 0.0, 1.0}};
    (*this).appliqueFiltre(kernel);
    (*this).EnregistrerImage("verticale.bmp");

    (*this).ptr = true_ptr;

    // Segmentation +45 degree
    kernel[0] = {new float[3]{0.0, 1.0, 2.0}};
    kernel[1] = {new float[3]{-1.0, 0.0, 1.0}};
    kernel[2] = {new float[3]{-2.0, -1.0, 0.0}};
    (*this).appliqueFiltre(kernel);
    (*this).EnregistrerImage("pos45deg.bmp");

    (*this).ptr = true_ptr;

    // Segmentation -45 degree
    kernel[0] = {new float[3]{-2.0, -1.0, 0.0}};
    kernel[1] = {new float[3]{-1.0, 0.0, 1.0}};
    kernel[2] = {new float[3]{0.0, 1.0, 2.0}};
    (*this).appliqueFiltre(kernel);
    (*this).EnregistrerImage("neg45deg.bmp");
...
// TP4 C++
// Modification pour traiter l'image en format bmp padding
Image2D::Image2D(char* path,double milx, double mily, bool rgb, bool padding){
    if(rgb && padding){
        FILE* f = fopen(path, "rb");
        //unsigned char en_tete[54];
        fread((*this).en_tete, sizeof(unsigned char), 54, f); // read the 54-byte header
...
// EnregistrerImage
void Image2D::EnregistrerImage(char* path, bool padding){
    if(padding){
       FILE* fp;
        fp = fopen(path, "wb");
        fwrite((*this).en_tete, 1, LONGUEUR_ENTETE, fp);
        for (int i=0; i<(*this).nby; i++) {
            ...
            unsigned char* padding = (unsigned char*) malloc(1);
            padding[0] = 0x00;
...
void Image2D::setPtr(int** tmp_ptr){
    (*this).ptr = tmp_ptr;
}

void Image2D::copyNewPtr(int** tmp_ptr){
    for (int i=0; i<(*this).getNby(); i++){
        for (int j=0; j<(*this).getNbx(); j++) {
            (*this).ptr[i][j] = tmp_ptr[i][j];
        }
    }
}
